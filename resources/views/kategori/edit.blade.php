        ...
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    {{-- Header: Navigasi Kembali dan Judul Halaman --}}
                    <div class="flex items-center gap-3 mb-6 border-b border-gray-200 dark:border-gray-700 pb-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-0.5">Perbarui nama kategori <span class="font-medium text-indigo-500">{{ $kategori->name }}</span></p>
                        </div>
                    </div>

                    {{-- Form Update: Mengirim data ke route update dengan Method Spoofing --}}
                    <form action="{{ route('kategori.update', $kategori->id) }}" method="POST" class="space-y-6">
                        @csrf {{-- Token keamanan Laravel --}}
                        @method('PUT') {{-- Mengubah POST menjadi PUT sesuai standar RESTful untuk update data --}}
                        ...
                        <div>
                            ...
                            {{-- Menampilkan pesan error jika validasi kolom 'name' gagal --}}
                            @error('name') 
                                <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span> 
                            @enderror
                        </div>

                        {{-- Action Buttons: Tombol Batal dan Update --}}
                        <div class="flex items-center justify-end gap-3 pt-4">
                            ...
                            <button type="submit" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-sm text-white hover:bg-indigo-700 transition ease-in-out duration-150">
                                Update Data
                            </button>
                        </div>
                    </form>
